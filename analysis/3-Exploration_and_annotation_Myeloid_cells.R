...
deg_myelo <- Seurat::FindAllMarkers(ds, 
                                    assay = "RNA", 
                                    logfc.threshold = 0.5, 
                                    min.pct = 0.2,
                                    only.pos = T,
...
clusters_annotation <-  c("Neutrophils 1", #1     
                          "CD16 Monocytes", #2
                          "Eosinophils", #3
                          "Neutrophils 2", #4
                          "CD14 Monocytes", #5
...
deg_myelo$Refined_annotation <- sapply(deg_myelo$cluster,
                                      function(x) look_clusters$Annotation[match(x, look_clusters$Cluster)])
# Set annotation order
annotation.levels <-  c("Neutrophils 1",
                        "Neutrophils 2",
                        "Eosinophils",
                        "CD16 Monocytes",
                        "CD14 Monocytes",
...
deg_myelo <- dplyr::arrange(deg_myelo, Refined_annotation) #reorder rows based on annotations
deg_myelo <- dplyr::select(deg_myelo, gene, cluster, Refined_annotation, p_val, p_val_adj, avg_log2FC, pct.1, pct.2) #reorder columns
deg_myelo <- dplyr::filter(deg_myelo, p_val_adj <= 0.05) #filter out adjusted p-value higher than 0.05
