...
# 2- Produce a table with cell counts for each bat individual
...
excluded_clusters <- c("Low-Quality 1",
                       "Low-Quality 2",
                       "Low-Quality 3",
                       "Doublets 1",
                       "Doublets 2",
...
for (ind in unique(data$Individual)) {
  tab <- data[which(data$Individual == ind),]
...
table$Age <- sapply(table$Individual, function(x) look_age$age[match(x, look_age$individual)])


table <- table[,c("Individual", 
                  "Age", 
                  "Neutrophils", 
                  "Eosinophils",
                  "CD16 Monocytes", 
...
#table[,-c(1,2)] <- apply(table[,-c(1,2)], MARGIN = c(1,2), function(x) if(is.na(x)) 0 else (x)) # Replace NAs by 0
